...
#### Where necessary, we change the name of some columns and change date to a date object. **Note** I have found Bike Count NA is represented by 0 and we will therefore remove Counts that are 0.
...
#### Create new variable called FullDate
...
#### Change the factor levels of Holiday and order, change the order of the Season factor levels. Within each Season we have "Yes" Holidays placed before "No" Holidays
...
#### Change units for specified columns
...
#### We need to change the class of some variables in the following pipeline so that we can use them when plotting graphs using ggplot.
