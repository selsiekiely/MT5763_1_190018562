...
### We can see that seasons do affect the average number of rented bikes. Within their daily Count proportions, both Seoul and Washington's Winter medians are considerably lower than the other seasons. In fact the range of Winter in Seoul is way below the lower quartile of the other seasons in Seoul. For both data sets we generally peak in Summer for Bike Counts and then trough in Winter. The count is much more spread in Autumn for both suggesting a varied uptake of bike rents. 
...
**Coefficients:** The Pr(>t) value for the coefficients in both data sets' linear models are generally below .05 therefore we can be confident about the fitting of the coefficients. The only exception to this is SeasonSummer in BikeSeoul which has Pr(>t) = 0.91247.
