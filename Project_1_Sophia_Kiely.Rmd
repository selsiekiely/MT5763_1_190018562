...
### I will compile Washington's Data such that the two years recorded overlap for plotting purposes.
```{r, include= FALSE}
...
ggplot(BikeWashington, aes(x = NewDate, y = Count, col = Season)) +
  geom_smooth() +
  labs(title = "Washington Average Bike Rent over Time", x = "Time in days", y = "Count", col = "Seasons")
```
...
### For Washington there is also a clear disinction 

### We could use group_by and summarise() in piping like below;
```{r, include=FALSE}
...
```
### However we would not be able to see the Season segments.
